...
function parseDependencies(requireify, fullPath, relativePath) {
    ...
    cleanedContent.replace(REPLACER, function(match, method, start, dependency) {
    ...
    content = content.replace(REPLACER, function(match, method, start, dependency, end) {
        var path = dependencies[dependency];

        if (!path && (path = nativeDependencies[dependency])) {
            return "__require__(" + start + path + end + ")";
        }

        return !!path ? method + "(" + start + path + end + ")" : match;
    });
